...
public class PetListFriendAda extends RecyclerView.Adapter<PetListFriendAda.petListDTOListHolder> {
    private Context mContext;
    ArrayList<PetListDTO> list;
    private ArrayList<PetListDTO> petListDTOList = null;
    LayoutInflater inflater;

    public PetListFriendAda(Context mContext, ArrayList<PetListDTO> petListDTOList, LayoutInflater inflater) {
    ...
    public petListDTOListHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.user_pet_profile_adapter, parent, false);
    ...
    public void onBindViewHolder(petListDTOListHolder mViewHolder, final int position) {
        try {
            ...
            mViewHolder.ctvdistance.setText(petListDTOList.get(position).getDistance() + " KM");
            ...
            if (petListDTOList.get(position).getVerified().equalsIgnoreCase("0")) {
                mViewHolder.ivVerify.setVisibility(View.GONE);

            } else {
                mViewHolder.ivVerify.setVisibility(View.VISIBLE);
            }
    ...
    public class petListDTOListHolder extends RecyclerView.ViewHolder {
        ...
        ImageView ivGender, ivVerify;
        CustomTextView ctvAge, ctvBreed, ctvCity, ctvdistance;
        ...
        public petListDTOListHolder(View item) {
            ...
            ctvdistance = (CustomTextView) item.findViewById(R.id.ctvdistance);
            ...
            ivVerify = (ImageView) item.findViewById(R.id.ivVerify);
        }
    }
        ...
        if (charText.length() == 0) {
            ...
            for (PetListDTO petListDTO : list) {
                if (petListDTO.getBreed_name().toLowerCase(Locale.getDefault())
                        .contains(charText) || petListDTO.getPetName().toLowerCase(Locale.getDefault())
                        .contains(charText) || petListDTO.getPet_type_name().toLowerCase(Locale.getDefault())
                        .contains(charText)) {
