...
import com.samyotech.petstand.adapter.AdapterCommentPetMarket;
...
public class CommentPetMarket extends AppCompatActivity implements View.OnClickListener {
    private String TAG = CommentPetMarket.class.getSimpleName();
    private RecyclerView lvChating;
    private AdapterCommentPetMarket adapterComment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ProjectUtils.setStatusBarGradiant(CommentPetMarket.this);
        setContentView(R.layout.activity_comment);
        mContext = CommentPetMarket.this;
        prefrence = SharedPrefrence.getInstance(mContext);
        loginDTO = prefrence.getParentUser(Consts.LOGINDTO);
        inputManager = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
        if (getIntent().hasExtra(Consts.POSTID)) {
            petId = getIntent().getStringExtra(Consts.POSTID);
        }
        init();
    }
...
//                overridePendingTransition(R.anim.pull_in_left, R.anim.push_out_right);
    ...
    public void getComment() {
        ...
        new HttpsRequest(Consts.GET_COMMENT_PET, getCommentParams(), mContext).stringPost(TAG, new Helper() {
            @Override
            public void backResponse(boolean flag, String msg, JSONObject response) {

                if (flag) {
                    Type listType = new TypeToken<List<CommentPetMarketDTO>>() {
                    }.getType();
                    try {
                        commentDTOList = new ArrayList<>();
                        commentDTOList = (ArrayList<CommentPetMarketDTO>) new Gson().fromJson(response.getJSONArray("data").toString(), listType);
    ...
    public Map<String, String> getCommentParams() {
        ...
        params.put(Consts.PET_ID, petId);
    ...
    public void showDataList() {
        ...
        adapterComment = new AdapterCommentPetMarket(CommentPetMarket.this, commentDTOList);
    ...
    public void doComment() {
        ProjectUtils.showProgressDialog(mContext, true, "Please wait....");
        new HttpsRequest(Consts.ADD_COMMENT_PET, doCommentParams(), mContext).stringPost(TAG, new Helper() {
            @Override
            public void backResponse(boolean flag, String msg, JSONObject response) {
